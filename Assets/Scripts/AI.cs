...
public class AI : MonoBehaviour
{
    ...
    private bool _isRandomlyPatrolling = false;
    private bool _hasIdled = false;
    ...
    void Update()
    {
        ...
        {
            if (!_isRandomlyPatrolling)
                StartCoroutine(PatrolRandom());
        }
    ...
    private void PatrolWaypoints()
    {
        ...
        if ((agent.hasPath && (agent.remainingDistance <= stoppingDistance)) || (waypoints[currentWaypointIndex].gameObject.activeSelf == false))
        {
            ...
            if (_hasIdled)
            {
                ...
                // If we are beyond number of waypointzs, set it to 0
    ...
    {
        //_isRandomlyPatrolling = true;

        Vector3 point;

        if (RandomPoint(transform.position, randomPatrolRadius, out point))
        {
            Debug.DrawRay(point, Vector3.up, debugPathColor, 1f);
            agent.SetDestination(point);
        }

        while (agent.hasPath && agent.remainingDistance <= stoppingDistance)
        {
